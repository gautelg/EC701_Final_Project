...
J_ca        = ca.diag([100., 100., 100.])
J_inv_ca    = ca.diag([0.01, 0.01, 0.01])
...
def make_physics_guess(x_cur, N):
    """
...
def build_opti(N):
    ...
    # Weights
    # Q_pointing is now 5x larger and Q_att_reg near-zero so
    # attitude regularisation no longer fights the pointing cost.
    Q_vel           = 50.0
    Q_omega         = 300.0
    R_thrust        = 80.0
    R_torque        = 150.0
    Q_pointing      = 25000.0   # was 8000  — dominant pointing term
    Q_pointing_rate = 10000.0   # was 3000  — smooth LOS tracking
    Q_att_reg       = 10.0      # was 150   — near-zero, don't fight pointing
    Q_qnorm         = 1500.0
    Q_du            = 80.0
    Q_dtau          = 400.0
    Q_wp_pos        = 800000.0
    ...
    # -------------------------------------------------------
    Q_pos_lateral = 2000.0   # x and z tracking (heavy)
    Q_pos_axial   = 200.0    # y tracking (lighter — allow free approach)
    ...
    # Control bounds
    for k in range(N):
        opti.subject_to(U[0:3, k] <= u_max_thrust / m_chaser)
        opti.subject_to(U[0:3, k] >= -u_max_thrust / m_chaser)
        opti.subject_to(U[3:6, k] <= tau_max)
        opti.subject_to(U[3:6, k] >= -tau_max)
...
opti, X_var, U_var, x0_p, wp_p, wpv_p = build_opti(N_leg)
...
for step in range(max_steps_total):
    ...
    if x_warm is not None:
        ...
        opti.set_initial(X_var, make_physics_guess(x, N_leg))
        opti.set_initial(U_var, np.zeros((6, N_leg)))
    ...
    try:
        ...
        print(f"  ⚠️  Warm-start failed at step {step}, WP{current_wp}: {str(e)[:80]}")
        try:
            opti.set_initial(X_var, make_physics_guess(x, N_leg))
            opti.set_initial(U_var, np.zeros((6, N_leg)))
            sol     = opti.solve()
            ...
            x_sol   = sol.value(X_var)
            u_apply = u_opt[:, 0]
            print(f"  ✅  Physics-guess retry succeeded at step {step}")
            solve_ok = False
        except RuntimeError as e2:
            print(f"  ❌  Physics-guess also failed: {str(e2)[:80]} — zero input")
            u_opt    = np.zeros((6, N_leg))
            x_sol    = make_physics_guess(x, N_leg)
            u_apply  = np.zeros(6)
            solve_ok = False
    ...
    d_body       = rotate_vector(x[6:10], los)
    cos_ang      = np.clip(np.dot([0., 0., 1.], d_body), -1., 1.)
    pointing_err = np.degrees(np.arccos(cos_ang))
    ...
    # Waypoint switching
    # WP_pre uses a lateral-only arrival check (x and z close to 0)
    if current_wp == 0:
        lateral_err = np.sqrt(x[0]**2 + x[2]**2)
        arrived = lateral_err < arrival_tol_pre
    else:
        arrived = np.linalg.norm(x[0:3] - wp_pos) < arrival_tol
...
np.save('angle_hist.npy',  angle_hist)
np.save('thrust_hist.npy', thrust_hist)
np.save('torque_hist.npy', torque_hist)
np.save('omega_hist.npy',  omega_hist)
np.save('vel_hist.npy',    vel_hist)
np.save('wp_visit_steps.npy', np.array(wp_visit_steps))
...
plt.tight_layout()
plt.savefig('mpc_waypoint_plots_lower_pointing.png', dpi=150, bbox_inches='tight')
plt.show()

# ================================================
# 3D Trajectory (close-up on the loop region)
# ================================================
...
# Circle waypoints only (skip WP_pre)
...
ax3d.set_title('3D Relative Trajectory — Full Mission')
ax3d.set_box_aspect([1, 1, 1])
ax3d.legend(fontsize=8)
ax3d.grid(True)

plt.savefig('mpc_waypoint_3d_lower_pointing.png', dpi=150, bbox_inches='tight')
plt.show()

# ================================================
# Bird's-eye view (x-y plane) — chaser path around target
# ================================================
fig_top, ax_top = plt.subplots(figsize=(8, 8))

# Full approach trajectory (grey, thinner)
ax_top.plot(pos_hist[:,1], pos_hist[:,0], color='lightsteelblue',
            lw=1.2, ls='--', label='Approach trajectory')

# Loop portion only — from WP_pre arrival onward
if len(wp_visit_steps) > 0:
    loop_start = wp_visit_steps[0]  # step when WP_pre reached
    ax_top.plot(pos_hist[loop_start:,1], pos_hist[loop_start:,0],
                'b-', lw=2.5, label='Loop trajectory')

# Nominal circle
theta_plt = np.linspace(0, 2*np.pi, 300)
ax_top.plot(circle_radius * np.cos(theta_plt),
            circle_radius * np.sin(theta_plt),
            'g--', lw=1.5, label=f'Nominal circle (r={circle_radius} m)')

# Keep-out circle (top-down cross-section)
ax_top.add_patch(plt.Circle((0, 0), keep_out_radius,
                             color='red', alpha=0.15, zorder=2))
ax_top.add_patch(plt.Circle((0, 0), keep_out_radius,
                             color='red', fill=False, lw=1.5,
                             ls='--', zorder=3,
                             label=f'Keep-out zone (r={keep_out_radius} m)'))

# Target
ax_top.plot(0, 0, 'r*', markersize=16, label='Target', zorder=5)

# Waypoints with arrival annotations
wp_colors_top = ['gold', 'magenta', 'limegreen', 'cyan']
for i, (wp, step) in enumerate(zip(waypoints[1:], wp_visit_steps[1:])):
    ax_top.scatter(wp[1], wp[0], color=wp_colors_top[i],
                   s=120, zorder=6, edgecolors='k', lw=0.8)
    ax_top.annotate(f'WP{i}\n(t={step*dt:.0f}s)',
                    xy=(wp[1], wp[0]),
                    xytext=(wp[1]+1.5, wp[0]+1.5),
                    fontsize=8, color='k',
                    arrowprops=dict(arrowstyle='->', color='k', lw=0.8))

# Starting position marker
ax_top.plot(pos_hist[0,1], pos_hist[0,0], 'k^',
            markersize=10, label=f'Start ({pos_hist[0,1]:.0f}, {pos_hist[0,0]:.0f}) m')

# Counterclockwise arrow hint
ax_top.annotate('', xy=(0, circle_radius+1.5), xytext=(circle_radius+1.5, 0),
                arrowprops=dict(arrowstyle='->', color='steelblue',
                                lw=2, connectionstyle='arc3,rad=-0.4'))
ax_top.text(circle_radius*0.6, circle_radius*0.6, 'CCW', fontsize=9,
            color='steelblue', ha='center')

ax_top.set_xlabel('y along-track [m]')
ax_top.set_ylabel('x radial [m]')
ax_top.set_title("Bird's-Eye View — Chaser Path Around Target (x-y plane)")
ax_top.set_aspect('equal')
ax_top.set_xlim(-25, 25)
ax_top.set_ylim(-25, 25)
ax_top.legend(fontsize=8, loc='upper left')
ax_top.grid(True, alpha=0.4)
ax_top.axhline(0, color='k', lw=0.5, alpha=0.3)
ax_top.axvline(0, color='k', lw=0.5, alpha=0.3)

plt.tight_layout()
plt.savefig('mpc_waypoint_birdseye.png', dpi=150, bbox_inches='tight')
plt.show()
